...
| Short-term cash yield exceeds equity earnings yield | Computed: SHY yield (or fed funds rate) vs. SPY earnings yield (1/PE) | `computed-mechanical` · Dependencies: SHY yield or fed funds rate (API), SPY PE (API) | SHY yield > SPY earnings yield | above | 0.15 | Binary threshold: when cash pays more than equities yield, the opportunity cost argument for equities collapses. No margin of safety when the risk-free alternative pays more. Recalibrate: this indicator auto-adjusts with rate changes — no manual recalibration needed. |
